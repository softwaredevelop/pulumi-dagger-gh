...
func TestNewStackInlineSourceActionsSecret(t *testing.T) {
	...
	qualifiedStackName := auto.FullyQualifiedStackName(orgName, prj.Name.String(), stackNameA)
	require.NotNil(t, qualifiedStackName)
	require.Equal(t, orgName+"/"+projectName+"/"+stackNameA, qualifiedStackName)
	...
	qualifiedStackName = auto.FullyQualifiedStackName(orgName, prj.Name.String(), stackNameB)
	require.NotNil(t, qualifiedStackName)
	require.Equal(t, orgName+"/"+projectName+"/"+stackNameB, qualifiedStackName)
	...
	s.Workspace().SetProgram(func(pCtx *pulumi.Context) error {
		...
		repositoryName := "pulumi-dagger-gh-test"
	...
	ss.Workspace().SetProgram(func(pCtx *pulumi.Context) error {
		...
		_, err = github.GetActionsPublicKey(pCtx, &github.GetActionsPublicKeyArgs{
			Repository: "pulumi-dagger-gh-test",
		}, nil)
		require.NoError(t, err)

		_, err = github.NewActionsSecret(pCtx, "newActionsSecret", &github.ActionsSecretArgs{
			Repository: pulumi.String("pulumi-dagger-gh-test"),
